...
- flow:
  - 外部機器がCPUに割り込み信号を送信
  - CPUは実行中のプロセスを一時中断し、対応する割り込みハンドラを実行
- ex. I/O、タイマー
- ref: [割り込み処理 - 筑波大学](http://www.coins.tsukuba.ac.jp/~yas/coins/os2-2012/2013-01-29/index.html)
...
- プロセス間通信の一種
- flow:
  - カーネル or プロセスがシグナルを作成、対象プロセスに送信
  - 対象プロセスは実行中の処理を一時中断し、対応するシグナルハンドラを実行
